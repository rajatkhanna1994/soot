...
public class LockAllocator extends SceneTransformer {
  ...
  protected void internalTransform(String phaseName, Map<String, String> options) {
    ...
    */
    ...
    if (!optionUseLocksets) {
      // deadlock detection for all single-lock-per-region allocations
    ...
    if (optionLeaveOriginalLocks) {
      ...
      // for locksets and dynamic locks
    ...
    if (optionUseLocksets) {
      // deadlock detection and lock ordering for lockset allocations
      ...
      for (SootClass appClass : Scene.v().getApplicationClasses()) {
        for (SootMethod method : appClass.getMethods()) {
          if (method.isConcrete()) {
            ...
            if (fs != null) {
              // (newly added methods need not be transformed)
  ...
  protected void findLockableReferences(List<CriticalSection> AllTransactions,
    ...
    while (tnIt9.hasNext()) {
      ...
      if (tn.group.useDynamicLock || tn.group.useLocksets) {
        // if attempting to use a dynamic lock or locksets
          ...
          if (tn.lockset == null || tn.lockset.size() <= 0) {
            ...
            for (EquivalentValue lockEqVal : tn.lockset) {
              ...
              if (lock instanceof Local) {
                ...
                // needs special treatment: could be primitive
                lockPT = null;
                ...
                // placeholder for anything that needs a static lock
                lockPT = null;
              } else {
                lockPT = null;
              }
              ...
              if (lockPT != null) {
                ...
                for (int i = 0; i < lockPTSets.size(); i++) {
                  ...
                  if (lockPT.hasNonEmptyIntersection(otherLockPT)) {
                    // will never happen for empty, negative numbered sets
                ...
                // static field locks and pathological cases...
                // Assign an existing lock number if possible
    ...
    if (optionUseLocksets) {
      ...
      for (int i = 0; i < lockPTSets.size(); i++) {
        ...
        if (pts.size() == 1 && false) {
          // isSingleton(pts)) // It's NOT easy to find a singleton: single alloc node must be run just once
  ...
  public void setFlagsForDynamicAllocations(CriticalSectionInterferenceGraph ig) {
    for (int group = 0; group < ig.groupCount() - 1; group++) {
      ...
      if (tnGroup.rwSet.size() <= 0) {
        // There are no edges in this group
        if (optionUseLocksets) {
      ...
      while (tnIt.hasNext()) {
        ...
        if (tn.setNumber == group + 1) {
          ...
          if (tn.group.useLocksets) {
            // print locks instead of dependence edges
            for (EquivalentValue lockEqVal : tn.lockset) {
